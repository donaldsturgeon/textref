...
sub updatecatalogdef
{
  ...
  my $sthcheck = $dbh->prepare("SELECT endpointid FROM catalogdef WHERE metaurl=?");
...
#  print $sql . "\n";
  ...
  foreach my $field (@datafields) {
    $storefield{$field->{'name'}} = 1;
#print "Will store " . $field->{'name'}" . \n";
  }
...
#    print "Parsing $line\n";
    if($csv->parse($line)) {
      ...
      for($i=0;$i<@columnbyfield;$i++) {
        if($columnbyfield[$i]) {
#          print "$columnbyfield[$i] => $fields[$i]\n";
